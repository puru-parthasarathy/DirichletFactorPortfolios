;; this file is used to get ftse100 data
(ns geteoddata.tickersFTSE
  (:require
...
;; get ftse100 - there is a manual step here, any ticker that ends with . the dot needs to be removed. Anything with .A will => -A.
;(def nasdaq100 (str/split-lines  (slurp (str HOME "/ftse100.txt"))))
(def nasdaq100 (str/split-lines  (slurp (str HOME "/ftse10020230622.txt"))))

(def trading-dates (map second (map first (cheshire/parse-string (:body
                                                                  (client/get (str
                                                                               "https://eodhistoricaldata.com/api/eod/RR.LSE?from=2001-01-01&to=2030-01-01&api_token=" api-token "&period=d&fmt=json")))))))
...
(defn trading-dates-after
  ...
  (map second (map first (cheshire/parse-string (:body (client/get
                                                        (str "https://eodhistoricaldata.com/api/eod/RR.LSE?from=" yyyy-mm-dd "&api_token=" api-token "&period=d&fmt=json")))))))
...
(defn eod-data-adapted
  ...
  ([ticker]
   (let [dummy-dates (zipmap (map keyword (trading-dates-after "2000-01-01")) (repeat nil))]
     ...
     (merge dummy-dates (eod-data :exchange "LSE" :ticker ticker)))))
...
;; Use Rolls Royce for FTSE 
(def data-available-till (final-date "RR"))
...
(when
 calculate-trading-universe?
  ...
  (def exchange-traded-0 (cheshire/parse-string (:body (client/get (format
                                                                    "https://eodhistoricaldata.com/api/exchanges/LSE?api_token=%s&fmt=json" api-token)))))

  (def permitted-exchanges '("BATS" "NYSE" "AMEX" "NASDAQ" "NYSE ARCA" "NYSE MKT" "IEX" "LSE"))
...
;; do a bulk last day call to which guys are trading in the list above
  (def latest-trading-data (cheshire/parse-string (:body (client/get
                                                          (str
                                                           "https://eodhistoricaldata.com/api/eod-bulk-last-day/LSE?api_token="
...
;; tickers that have market cap higher than 300 and price greater than 10
  ...
  (def opens-market (eod-data-tickers  '("RR")))
...
(defn exec-1[]
  (pprint "Working on FTSE100 stocks...long history")
(def opens-updated
...
;/home/psarathy/data/soup
;/home/psarathy/data/owner
;/home/psarathy/data/theory
