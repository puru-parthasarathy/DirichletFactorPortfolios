...
;; doesnt work need funda data access
;(client/get "https://eodhistoricaldata.com/api/fundamentals/GSPC.INDX?api_token=5ffde981a31603.50022638" )


;; get nasdaq100 tickers from a flat file - outdated
;(def nasdaq100 (str/split-lines  (slurp (str HOME "/nasdaq100Tickers.txt"))))


;; get nasdaq100 tickers from a flat file as of June 6th 2023, 101 tickers
...
;;; functions that retrieve data from the eod historical data API


(pprint
(format (str "https://eodhistoricaldata.com/api/eod/" "HOG" "." "NASDAQ" "?from=%s&api_token=" api-token "&period=d&fmt=json") "2000-01-01") 
 )
...
(defn eod-data-adapted
  ...
  ([ticker]
   (let [dummy-dates (zipmap (map keyword (trading-dates-after "2000-01-01")) (repeat nil))]
     ...
     (merge dummy-dates (eod-data :exchange "COMM" :ticker ticker)))))
...
(def data-available-till (final-date "ES"))
...
(eod-data-tickers ["HOG"])



;;; functions below clean and organize the data fetched above
...
(def calculate-trading-universe? false)
...
;; tickers that have market cap higher than 300 and price greater than 10
  ...
  (def opens-market (eod-data-tickers  '("ES")))
...
;; only do these two
(def trading-universe nasdaq100)
...
(pprint "***************Calling Main...***********************")
  ...
  (def opens-updated
    (eod-data-tickers (take 50 trading-universe)))


(eod-data-tickers ["MSFT" "AAPL"])
